...
# Step 6: Weather Impact on Traffic
plt.figure(figsize=(8,5))
weather_congestion = traffic_df.groupby("Weather Condition")["Traffic Index"].mean().sort_values(ascending=False)
sns.barplot(x=weather_congestion.index, y=weather_congestion.values, palette="viridis")
plt.title("Average Traffic Index by Weather Condition")
plt.xlabel("Weather Condition")
plt.ylabel("Traffic Index")
plt.tight_layout()
plt.show()

# Step 7: Congestion Trends Over Time
daily_avg_traffic = traffic_df.groupby("Date")["Traffic Index"].mean()
plt.figure(figsize=(12,5))
sns.lineplot(x=daily_avg_traffic.index, y=daily_avg_traffic.values, color="darkred")
plt.title("Daily Traffic Congestion Trend")
plt.xlabel("Date")
